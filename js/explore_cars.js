// explore_cars.js — unified, responsive page size (8 on mobile, 9 otherwise)
document.addEventListener("DOMContentLoaded", () => {
  ...
  const MOBILE_PAGE   = 8;
  const DESKTOP_PAGE  = 9;
  const mq            = window.matchMedia("(max-width: 575.98px)");
  let PAGE_SIZE       = mq.matches ? MOBILE_PAGE : DESKTOP_PAGE;

  mq.addEventListener("change", () => {
    PAGE_SIZE = mq.matches ? MOBILE_PAGE : DESKTOP_PAGE;
    renderCurrent(); // re-apply pagination with the new page size
  });
  ...
  function populateModelYear() {
    ...
    resetSelect(modelSelect, "Model"); models.forEach(m => appendOption(modelSelect, m, m));
    resetSelect(yearSelect,  "Year");  years.forEach(y => appendOption(yearSelect,  y, y));
  }
  ...
  function cardMatches(card, top, aside) {
    ...
    for (const [key, set] of Object.entries(aside)) {
      ...
      if (key === "features") {
        ...
        let ok = false; for (const want of set) if (feats.includes(want)) { ok = true; break; }
        if (!ok) return false;
      } else if (!set.has(val)) {
        return false;
      }
  ...
  if (aside) {
    aside.addEventListener("change", (e) => {
      const t = e.target;
      if (t && (t.matches('input[type="checkbox"]') || t.matches('input[type="radio"]'))) renderCurrent();
    });
  }
  ...
  loadMoreBtn && loadMoreBtn.addEventListener("click", (e) => {
    ...
    const currentlyVisible = included.filter(c => c.style.display !== "none").length;
